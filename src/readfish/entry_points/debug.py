...
from readfish._raijin_client import RaijinClient
...
CLASS_MAP = {
    65: "adapter", 66: "mux_uncertain", 67: "unknown_negative",
    68: "unknown_positive", 69: "event", 77: "multiple",
    78: "unclassed", 80: "pore", 83: "strand",
    84: "transition", 85: "unavailable", 90: "zero"
}
...
class Analysis:
    ...
    def run(self):
        ...
        self.chunk_log_file = None
        self.chunk_tsv_writer = None
        self.debug_log = True
        chunk_log_name = f"{self.run_information.run_id}_chunks_debug.tsv"
        self.chunk_log_file = open(chunk_log_name, 'w', newline='')
        self.chunk_tsv_writer = csv.writer(self.chunk_log_file, delimiter='\t')
        self.chunk_tsv_writer.writerow(['read_id', 'chunk_nr', 'sample_count', 'classifications'])
        ...
        while self.client.is_sequencing:
            ...
            for channel, read in chunks:
                rid = read.id
                self.read_chunk_counts[rid] = self.read_chunk_counts.get(rid, 0) + 1
                chunk_nr = self.read_chunk_counts[rid]
                signal = np.frombuffer(read.raw_data, dtype=np.int16)
                class_names = [CLASS_MAP.get(c, str(c)) for c in read.chunk_classifications]

                if self.chunk_tsv_writer:
                    self.chunk_tsv_writer.writerow([rid, chunk_nr, len(signal), ",".join(class_names)])

                chunks_to_basecall.append((channel, read))
            
            calls = self.caller.basecall(
                chunks_to_basecall, self.client.signal_dtype, self.client.calibration_values
            )
